...
Gui, Font, s11, Segoe UI
Gui, Add, Text, xs+10 yp+30, #2 Button position for Athanor improvement:
Gui, Font, s9
...
Gui, Add, Button, x+10 w170 h35 gDust_convert, FAST DUST CONVERT
...
Gui, Font
Gui, Font, s10
Gui, Add, Text, cBlue xs+50 yp+30, Stop = F1
Gui, Add, Text, cBlue x+15, Pause = F2
Gui, Add, Text, cBlue x+15, Restart / Reset = F3
Gui, Add, Text, cRed xs+60 yp+30, Open Chests = F5
Gui, Add, Text, cRed x+10, Salvage Items = F7
Gui, Add, Text, cRed xs+100 yp+30, Salvage Red Items = F9
Gui, Add, Text, cRed xs+70 yp+30, Fast Dust Convert / Item Crafting = F11
Gui, Add, Text, cRed xs+50 yp+30, Upgrade Atanor = F12
Gui, Add, Text, cMaroon x+10, Close Script = Ctrl+Esc
Gui, Font
Gui, Show, w400 h780, SUSik
...
loop {
    ...
    Send, {Space}
    CycleCount++
    ToolTip, Running... Iteration: %CycleCount%/%LoopCount%
}
ToolTip
...
return
    ...
    ToolTip, Click LMB to set`nPress ESC to cancel`n`nCurrent coordinates: X=%currentX% Y=%currentY%
...
IniWrite, %coord1%, %A_AppData%%basename1%\presets.ini, %frmSAVEDPRESET%, Pos_%A_Index%X
IniWrite, %coord2%, %A_AppData%%basename1%\presets.ini, %frmSAVEDPRESET%, Pos%A_Index%_Y
}
}
