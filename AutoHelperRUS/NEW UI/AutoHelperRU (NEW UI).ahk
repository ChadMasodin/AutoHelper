...
;CoordMode, Mouse, Client ; Определяет координаты мыши относительно клиентскойобласти окна
...
CreateGUI:
    ...
    Gui, Add, GroupBox, x10 y40 w380 h300 Center Section, Настройка скриптов
    Gui, Font
    ...
    Gui, Add, Text, x25 y580, Общий ограничитель циклов (опционально):
    Gui, Add, Edit, x+10 w75 h20 vLoopCount Number

    Gui, Font, s9 Bold, Segoe UI
    Gui, Add, Button, x25 y610 w130 h30 gSetLocation, Установите окно:
    Gui, Add, Edit, x+15 yp+3 w210 h25 vTarget_Window ReadOnly, %Target_Window%
    Gui, Font

    ; Action Buttons Section
    Gui, Font, c535353 s13 Bold Q3, Segoe UI Black
    Gui, Add, GroupBox, x10 y350 w380 h210 Center Section, Кнопки
    Gui, Font, s9 Bold, Segoe UI
...
SalvageRedItems:
    ...
    if (!ValidateScript()) {
        LogAction("Утилизация красных предметов: ошибка валидации")
        return
    }
